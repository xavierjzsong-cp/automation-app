...
src/ui/app.py
    Partial legacy CustomTkinter application shell with window setup, static layout, widget factories, settings persistence, file/folder browse callbacks, and target-sheet filtering; the entry point remains inactive until the remaining callbacks are restored.
...
scripts/check_ui_app_shell.py
    Import-safe application class, static layout, settings persistence, browse callbacks, target-sheet filtering, method signature, entry-point boundary, Playwright path, and repeatability check without creating a window.
